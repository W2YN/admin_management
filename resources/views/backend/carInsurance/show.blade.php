...
@section('content')
    ...
    <div class="page-container">
        ...
        <div class="btn-group" style="padding: 10px 0;">
            ...
            <!--<a href="frame_window_open('保单金额','','200','200')" style="margin-left:10px;"><span class="btn btn-primary radius">保单金额</span></a>-->
            <a href="{{route('backend.carInsurance.order.download', ['id'=>$data->id])}}" target="_blank"
               style="margin-left:10px;"><span
                        class="btn btn-default radius">客户资料下载</span></a>
            @if($data->waitGet)
                <a title="领取" href="javascript:;" onclick="takeOrder(this,'{{$data->id}}')" style="margin-left: 10px;"
                   data-url="{{url(route('backend.carInsurance.order.takeOrder',['id'=>$data->id]))}}"
                   data-formid="del-from-1"
            ...
            @else
                @if($data->status==1 or $data->status==0)
                    <a title="" href="javascript:;" onclick="setStatus(2)" style="margin-left: 10px;"
                       class="btn btn-success radius">
                        设为处理中
                    </a>
                @elseif($data->status==2)
                    <a title="" href="javascript:;" onclick="setStatus(3)" style="margin-left: 10px;"
                       class="btn btn-success radius">
                        设为等待支付
                    </a>
                @elseif($data->status==3)
                    <a title="" href="javascript:;"  style="margin-left: 10px;"
                       class="btn btn-success radius">
                        <!-- 这里还是有些疑问的 -->
                        等待用户填写支付信息
                    </a>
                 @elseif($data->status==4)
                    <a title="" href="javascript:;"  onclick="startPay()"  style="margin-left: 10px;"
                       class="btn btn-success radius">
                        <!-- 这里还是有些疑问的 -->
                        发起支付请求
                    </a>
                 @elseif($data->status==5)
                    <!-- 这里就不显示了 !!!!!!!!-->
                    <!--<span class="btn btn-success radius">首次支付已完成</span>-->
                @endif
            @endif
            <-- 下面是更多的可以添加的按钮 -->
